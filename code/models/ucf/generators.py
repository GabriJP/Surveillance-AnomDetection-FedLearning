...
class FeatureGenerator(Sequence):
	...
	def __init__(self, normal_src_dir: list or tuple or str,
		...
		# Check input
		if not instance(normal_src_dir, (list, tuple, str)):
			raise TypeError('"normal_src_dir" must be a str or a'\
								' collection of str')

		if (instance(normal_src_dir, (list, tuple)) and
							any(not instance(x, str) for x in normal_src_dir)):
			raise TypeError('Any source dir in "normal_src_dir" not a valid str')

		if not instance(anormal_src_dir, (list, tuple, str)):
			raise TypeError('"anormal_src_dir" must be a str or a'\
								' collection of str')

		if (instance(anormal_src_dir, (list, tuple)) and
							any(not instance(x, str) for x in anormal_src_dir)):
		...
		if not self.__normal_vid_info:
			raise RuntimeError('No normal video found on the specified'\
																' directories')

		if not self.__anormal_vid_info:
			raise RuntimeError('No anormal video found on the specified'\
																' directories')
	...
	def __getitem__(self, idx) -> np.ndarray:
		...
		if self.__normal_vid is None or
									(vid_idx >= self.__loaded_vid_range[0] and
										vid_idx < self.__loaded_vid_range[1])):
			...
			self.__normal_vid = np.concatenate([
				FeatureGenerator.__load_vid_feat(self.__acc_normal_vid_info[i],
													self.__feat_size)
									for i in range(self.__loaded_vid_range[0],
												self.__loaded_vid_range[1])
										])
