...
@description: Trains and evaluate an Incremental Spatio Temporal Learner (ISTL)
				model by using the UCSD Ped 1 and UCSD Ped2 train and test sets
				on a separated nodes of a federated architecture simulated on a
				single node through many active learning phases.

			This scripts replicates the experiment carried out on [1] for the
			UCSD Ped dataset adding the federated learning architecture in
...
@usage: train_ISTL_UCSDPed1&2.py -d <JSON document experiment> [-s]
"""
