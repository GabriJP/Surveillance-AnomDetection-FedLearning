...
class FedLearnModel:
	...
	def __init__(self, build_fn, n_clients: int):

		self._build_fn = build_fn
		self._n_clients = n_clients

		# Global model
		self._global_model = self._build_fn()

		# The clients model
		self._client_model = dict(zip(range(self._n_clients),
							(self._build_fn() for i in range(self._n_clients))))
...
class SynFedLearnModel(FedLearnModel):
	...
	def __init__(self, build_fn, n_clients: int):
		super(SynFedLearnModel, self).__init__(build_fn, n_clients)
...
class SynFedAvgLearnModel(SynFedLearnModel):
	...
	def __init__(self, build_fn, n_clients: int):
		super(SynFedAvgLearnModel, self).__init__(build_fn, n_clients)
	...
	def fit(self, **kwargs):
		...
		for epoch in range(n_epochs):
			...
			for c in self._client_model:
				_copy_weights(self._global_model, self._client_model[c])

			# Perform local training
			for c in self._client_model:

				if kwargs['x'][c] is None or kwargs['y'][c] is None:
					continue
				...
				hist = self._client_model[c].fit(
				    ...
				    y=kwargs['y'][c] if isinstance(kwargs['y'], dict) else kwargs['y'],
				    batch_size=kwargs['batch_size'],
				    epochs=1,
				    verbose=kwargs['verbose'],
				    callbacks=None,
				    validation_split=kwargs['validation_split'],
				    validation_data=kwargs['validation_data'][c] if isinstance(kwargs['validation_data'], dict) else kwargs['validation_data'],
				    ...
				    validation_batch_size=kwargs['validation_batch_size'],
				    validation_freq=kwargs['validation_freq'],
				...
				for h in hist.history:
					history[c][h][epochs] = hist.history[h][0]

				# Execute early stopping schedule
				if (np.abs(history[c][self.__early_stop['monitor']][epochs] -
							history[c][self.__early_stop['monitor']][epochs-1]) <=
